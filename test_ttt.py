...
if __name__ == '__main__':
    ...
    # TTT的学习率   learning rate

    LR = 1e-3

    # 更新均值方差   uopdate the mean value
    ...
    # 将backbone中的所有batchnorm层的参数获取出来，放到param_dicts中，lr=0.0001   Get the parameters of all batchnorm layers in the backbone and put them in param_dicts, lr=0.0001

    param_dicts = [

        {"params": [p for n, p in model.named_parameters() if (".bn"  in n or (("final_layer" in n or "header" in n) and (".1.weight"  in n or ".1.bias"  in n or ".4.weight"  in n or ".4.bias"  in n)) ) and p.requires_grad]}

    ]

    

    opt = torch.optim.Adam(param_dicts, lr=LR)
